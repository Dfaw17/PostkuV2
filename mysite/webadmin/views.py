...
def home(requets):
    ...
    pegawai = Account.objects.filter(is_owner=0)
    total_pegawai = pegawai.count()

    transaction = Transaction.objects.aggregate(Sum('total'))
    if transaction.get('total__sum') == None:
        total_all_transaction = 0
    else:
        total_all_transaction = transaction.get('total__sum')

    transaction_tunai = Transaction.objects.filter(payment_type_id=1).aggregate(Sum('grand_total'))
    if transaction_tunai.get('grand_total__sum') == None:
        total_all_trx_tunai = 0
    else:
        total_all_trx_tunai = transaction_tunai.get('grand_total__sum')

    transaction_qris = Transaction.objects.filter(payment_type_id=2).aggregate(Sum('total'))
    if transaction_qris.get('total__sum') == None:
        total_all_trx_qris = 0
    else:
        total_all_trx_qris = transaction_qris.get('total__sum')

    revenue_postku = Transaction.objects.aggregate(Sum('pajak'))
    if revenue_postku.get('pajak__sum') == None:
